 ...
 * start typing immediately without anyone tapping "Next" first — the
 * result panel just updates in place below it.
 */
export default function KioskPage() {
  ...
  return (
    <div className="relative flex min-h-svh w-full flex-col items-center bg-background px-lg py-xl overflow-y-auto">
      <button
        onClick={() => navigate("/")}
      ...
      <div className="w-full max-w-[26rem] text-center my-auto">
            ...
            {result.ok ? (
              <>
                <div className="text-center">
                  <Icon name="account_circle" className="!text-4xl text-primary-container" />
                  <h2 className="mt-xs font-headline text-headline-md font-bold text-on-surface">{result.data.fullName}</h2>
                  <p className="font-label-md text-label-md text-on-surface-variant">Member #{result.data.memberCode}</p>
                </div>

                <div className={`mt-md rounded-xl px-lg py-lg text-center ${FEE_BANNER[result.data.fee.state].className}`}>
